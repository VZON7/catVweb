...
⚠️ 这一节每一条都对应一次真实的数据丢失。**动同步代码前必读，三道保险一条都不能拆。**

### 保险一：上传安全闸 `wouldLoseRecords()`

**整包同步的致命缺陷：上传是无条件覆盖。任何一个本地为空的客户端，同步一次就能把云端清空。** 这不是 bug，是设计使然。

规则：推之前数一数「云端有、我没有、我的删除名单里也没有」的记录。这种记录代表**别的设备写的、我根本没见过**，推上去就是销毁。数量 > 0 就拒绝上传。

自己删掉的在删除名单里，不会误判。合并正常完成时这个数必然是 0，所以正常流程不受影响。

### 保险二：上传后必须读回核对

`upsert` 时带 `.select('data')`，拿服务器实际存下的内容跟本机比条数，对不上就报错。

**踩过的坑：上传返回成功、零报错、界面一路显示「已同步」，实际什么都没存进去，直到换设备才发现全空。** 静默丢数据是最恶劣的故障，宁可报错也不能假装成功。

### 保险三：换账号先备份再让位

换账号时本机数据要让位给云端那份，但它可能从来没上传过。**让位前必须存进 `cj_switch_backup`（标准备份格式，可从「导入」贴回来），存不下就中止同步。**

原则：**任何情况下都不允许「无备份地丢弃本地数据」。**

### 还有两个必须保持的写法

**同步锁 `_inSync` 必须在 `finally` 里释放。** 曾经写在 `try` 内部，中间一句抛错就永远卡住 → `schedulePush` 和 `cloudSync` 全部第一行 return → 上传彻底失效，而圆点一直是绿的，完全看不出来。另外配了 30 秒强制解锁兜底。

**同步开始就把 `cloudUser.id` 抓进局部变量 `uid`。** 中途用户登出会让 `cloudUser` 变 null，后面再读 `.id` 直接崩，这次同步的数据就没了。

### 调试手段

同步每次都在控制台留痕：`[sync] 完成 · 记录 N 条（服务器已核对 M 条）`。失败是 `[sync] 失败: 原因`。**排查同步问题先看这两行，别猜。**

数据库侧对账 SQL：

```sql
select u.email,
       jsonb_typeof(v.data->'entries') as t,
       left(v.data->>'entries', 400)   as 原文,
       v.updated_at
from vaults v join auth.users u on u.id = v.user_id
