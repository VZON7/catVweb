...
| 同步弹窗：「立即同步」点完关窗、「一并清除」前先确认推成功、「记住我」只记邮箱、邮箱按 Enter 跳密码 | 第225次 |
| 改密码（先验当前密码再改）；修「取消勾选『记住我』邮箱框不清空」 | 第226次 |
| 同步弹窗文案：改密码入口改成整块按钮、「登出」改「同步并登出」、登出二选一补「是不是自己的设备」判断标准 | 第227次 |
| 同步弹窗层级重排：改密码/登出降为并排次要按钮、主按钮只留「立即同步」；登出二选一改成「场景当标题」 | 第228次 |
